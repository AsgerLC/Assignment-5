...
- Why can we include the studies where the pitch is measured in semitones?

##### THINGS THAT NEED TO BE RESEARCHED

- What does adding a fixed effect / moderator mean to an RMA? What is a Test of Moderators?
Moderator = interaction effect. 
Test of moderators = Wald-type chi-square test with X degrees of freedom, testing if the effect of the moderator is significant.  
- How do we interpret all the fancy numbers in an influence() test? Can we look up the function? What creates the "inf" statistic (the one that shows a *)?
- Model comparison between model with or without random effects (though, since it is an explanatory model, maybe it isn't that big of a deal?)
- How the freakin' hell do you interpret forest plots? With and without creepy shadows?
